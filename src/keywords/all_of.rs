    ...
    if let Some(Value::Array(items)) = schema_object.get_mut("allOf") {
        ...
        if items.is_empty() {
            ...
            for primitive_types in &all_of_primitive_types {
                ...
                if !primitive_types.is_complete() {
            ...
            if !defined_types_in_all_of {
                ...
                updated_schema |= replace::type_with(schema_object, common_all_of_primitive_types);
            }
        }
        updated_schema
    } else {
...
mod tests {
    ...
    #[test_case(&json!({"allOf": [{"type": "string"}]}) => json!({"allOf": [{"type": "string"}], "type": "string"}))]
    #[test_case(&json!({"allOf": []}) => json!({"allOf": []}))]
    #[test_case(&json!({"type": "object", "allOf": [{}]}) => json!({"type": "object"}))]
    #[test_case(&json!({"allOf": [false]}) => json!(false))]
    #[test_case(&json!({"allOf": [{"type": ["integer", "string"]}]}) => json!({"allOf": [{"type": ["integer", "string"]}], "type": ["integer", "string"]}))]
    #[test_case(&json!({"allOf": [{"type": "integer"}, {"type": "number"}]}) => json!({"allOf": [{"type": "integer"}, {"type": "integer"}], "type": "integer"}))]
    #[test_case(&json!({"allOf": [{"type": ["integer", "string"]}, {"type": "number"}]}) => json!({"allOf": [{"type": "integer"}, {"type": "integer"}], "type": "integer"}))]
    #[test_case(&json!({"allOf": [{"type": "string"}, {"type": "number"}]}) => json!(false))]
    #[test_case(&json!({"allOf":[{"type":"integer"}], "type": "boolean"}) => json!(false))]
    fn test_simplify_all_of(schema: &Value) -> Value {
        crate::base_test_keyword_processor(&simplify_all_of, schema)
    }
