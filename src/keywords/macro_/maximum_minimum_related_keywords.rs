use crate::helpers::{replace, types::get_primitive_types};
...
fn cleanup_incongruent_keywords(
    schema: &mut Value,
    schema_primitive_types: &BTreeSet<PrimitiveType>,
    should_replace_with_false_schema: fn(&BTreeSet<PrimitiveType>) -> bool,
    evaluate_final_primitive_types: fn(&BTreeSet<PrimitiveType>) -> BTreeSet<PrimitiveType>,
        ...
        || schema_primitive_types.contains(&PrimitiveType::Integer)
    {
        match (
            ...
            (Some(max_), Some(min_)) if max_ < min_ => cleanup_incongruent_keywords(
                ...
                |schema_primitive_types| {
                    schema_primitive_types.len() == 1
                        || (schema_primitive_types.len() == 2
                            && schema_primitive_types.contains(&PrimitiveType::Number)
                            && schema_primitive_types.contains(&PrimitiveType::Integer))
                },
                |schema_primitive_types| {
                    let mut result = schema_primitive_types.clone();
                    let _ = result.remove(&PrimitiveType::Integer);
                    let _ = result.remove(&PrimitiveType::Number);
                    result
                },
    ...
    if schema_primitive_types.contains(&PrimitiveType::Array) {
        match (
            ...
            (Some(max_), Some(min_)) if max_ < min_ => cleanup_incongruent_keywords(
                schema,
                schema_primitive_types,
                |schema_primitive_types| schema_primitive_types.len() == 1,
                |schema_primitive_types| {
                    let mut result = schema_primitive_types.clone();
                    let _ = result.remove(&PrimitiveType::Array);
                    result
                },
                &["maxItems", "minItems"],
            ),
    ...
    if schema_primitive_types.contains(&PrimitiveType::String) {
        match (
            ...
            (Some(max_), Some(min_)) if max_ < min_ => cleanup_incongruent_keywords(
                ...
                |schema_primitive_types| {
                    let mut result = schema_primitive_types.clone();
                    let _ = result.remove(&PrimitiveType::String);
                    result
                },
                &["maxLength", "minLength"],
            ),
    ...
    if schema_primitive_types.contains(&PrimitiveType::Object) {
        match (
            ...
            (Some(max_), Some(min_)) if max_ < min_ => cleanup_incongruent_keywords(
                ...
                |schema_primitive_types| {
                    let mut result = schema_primitive_types.clone();
                    let _ = result.remove(&PrimitiveType::Object);
                    result
                },
...
pub(crate) fn update_min_max_related_keywords(schema: &mut Value) -> bool {
    ...
    if schema.get("type").is_some() {
        let schema_primitive_types = get_primitive_types(schema.get("type"));
...
mod tests {
    ...
    use crate::helpers::types::get_primitive_types;
    use crate::primitive_type::PrimitiveType;
    ...
    fn test(
        keyword_update_logic: fn(&mut Value, &BTreeSet<PrimitiveType>) -> bool,
        mut schema: Value,
    ) -> Value {
        crate::init_logger();
        let schema_primitive_types = get_primitive_types(schema.get("type"));
    ...
    fn test_update_min_max_related_keywords_does_not_perform_modifications_if_missing_or_incongruent_type(
        mut value: Value,
    ) {
        crate::init_logger();
        let initial_value = value.clone();
        assert!(!update_min_max_related_keywords(&mut value));
    ...
    #[test_case(json!({"type": ["integer", "null"], "maximum": 1, "minimum": 2}) => json!({"type": "null"}))]
    #[test_case(json!({"type": ["null", "number"], "maximum": 1, "minimum": 2}) => json!({"type": "null"}))]
    fn test_update_min_max_related_keywords_does_performs_modifications(mut value: Value) -> Value {
        crate::init_logger();
        assert!(update_min_max_related_keywords(&mut value));
        value
    }
